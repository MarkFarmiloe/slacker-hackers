...
function ROW(prop){
    ...
    if(parseInt(prop.obj.posts)<5){
        return(<tr style={{backgroundColor:'#eb5a46'}}><td>{prop.obj.name}</td><td>{prop.obj.location}</td><td>{prop.obj.class}</td><td>{parseInt(prop.obj.posts)}</td><td>{prop.obj.week}</td><td><button type="button" onClick={warnFunc} className="btn btn-danger">Profile</button></td></tr>
       )
    }else if(parseInt(prop.obj.posts)<10){
        return(
            <tr style={{backgroundColor:'orange'}}><td>{prop.obj.name}</td><td>{prop.obj.location}</td><td>{prop.obj.class}</td><td>{prop.obj.posts}</td><td>{prop.obj.week}</td><td><button type="button" onClick={warnFunc} className="btn btn-warning">Profile</button></td></tr>
        )
    }else{
        return(
            <tr style={{backgroundColor:'#90ee90'}}><td>{prop.obj.name}</td><td>{prop.obj.location}</td><td>{prop.obj.class}</td><td>{prop.obj.posts}</td><td>{prop.obj.week}</td><td><button type="button" onClick={warnFunc}  className="btn btn-success">Profile</button>
            </td></tr>
